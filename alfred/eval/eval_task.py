...
def evaluate_task(
    ...
    if not args.meta_action:
        ...
        vocab = {'word': dataset.vocab_in, 'action_low': model.vocab_out, 'meta_action': model.meta_action_vocab}
    ...
    init_position = env.last_event.metadata['agent']['position']
    init_rotation = env.last_event.metadata['agent']['rotation']
    last_orientation = (init_position['x'], init_position['y'], init_position['z'], round(init_rotation['y']))
    last_action = 'LookDown_15'
    last_action_timestep = 0
    last_action_teleport = False
    while t < args.max_steps:
        # get an observation and do an agent step
        input_dict['frames'] = eval_util.get_observation(env.last_event, extractor)
        current_position = env.last_event.metadata['agent']['position']
        current_rotation = env.last_event.metadata['agent']['rotation']
        agent_location.append((current_position['x'], current_position['z']))
        if not args.meta_action:
            if not last_action_teleport:
                ...
                episode_end, prev_action, num_fails, _, _ = eval_util.agent_step(
                    model, input_dict, vocab, prev_action, env, args, num_fails, obj_predictor, None, last_action_timestep)
                
        else:
            episode_end, prev_action, prev_meta_action, num_fails, _, _ = eval_util.agent_meta_step(
                model, input_dict, vocab, prev_action, prev_meta_action, env, args, num_fails, obj_predictor)
    ...
    metrics['agent_location'] = agent_location
