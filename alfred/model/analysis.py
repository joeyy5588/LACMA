...
def prepare(train, exp):
    ...
    args.dout = os.path.join(constants.LACMA_LOGS, args.name)
...
def load_data(name, args, ann_type):
    '''
...
def main(train, exp):
    ...
    # start train loop
    seen_loader = loaders['valid_seen:lmdb_human;lang']
    unseen_loader = loaders['valid_unseen:lmdb_human;lang']

    # eval
    model.eval()

    for loader in [seen_loader, unseen_loader]:
        vocab_in = loader.dataset.vocab_in
        img_feat_list = []
        txt_feat_list = []
        contrastive_label_list = []
        curr_label_max = 0
        for batch_idx, batch in tqdm(enumerate(loader), total=len(loader)):
            traj_data, input_dict, gt_dict = data_util.tensorize_and_pad(batch, args.device, 0)
            with torch.no_grad():
                model_out = model.model(vocab_in, meta_action=gt_dict['meta_action'], action=gt_dict['action'], 
                                meta_sentnum=gt_dict['meta_sentnum'], meta_boundary=gt_dict['meta_boundary'],
                                **input_dict)

                if 'contrastive_label' not in model_out:
                    continue
                contrastive_logits = torch.matmul(model_out['img_feat'], model_out['txt_feat'].transpose(0, 1))
                labels = model_out['contrastive_label']
                preds = contrastive_logits.argmax(dim=1)
                # print(preds.shape, labels.shape, model_out['txt_feat'].shape, contrastive_logits.shape)
                # labels = torch.arange(contrastive_logits.shape[0]).to(contrastive_logits.device)
                correct = (labels == preds).float().sum()
                print(correct, correct / labels.shape[0])

                img_feat = model_out['img_feat'].cpu()
                txt_feat = model_out['txt_feat'].cpu()
                contrastive_label = model_out['contrastive_label'].cpu()
                assert txt_feat.shape[0] > contrastive_label.max().item(), (txt_feat.shape, contrastive_label.max().item())
                temp_max = txt_feat.shape[0]
                contrastive_label += curr_label_max
                curr_label_max += temp_max
                torch.cuda.empty_cache()
                img_feat_list.append(img_feat)
                txt_feat_list.append(txt_feat)
                contrastive_label_list.append(contrastive_label)

        img_feat = torch.cat(img_feat_list, dim=0)
        txt_feat = torch.cat(txt_feat_list, dim=0)
        contrastive_label = torch.cat(contrastive_label_list, dim=0)

        torch.save({'img_feat': img_feat, 'txt_feat': txt_feat, 'contrastive_label': contrastive_label},
                    os.path.join('data/probe', f'{loader.dataset.partition}_orig_32.pt'))