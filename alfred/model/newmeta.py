import torch
from torch import nn
from torch.nn import functional as F

from alfred.model import base
from alfred.nn.enc_lang import EncoderLang
from alfred.nn.enc_visual import FeatureFlat
from alfred.nn.enc_vl import EncoderVL
from alfred.nn.encodings import DatasetLearnedEncoding
from alfred.nn.dec_object import ObjectClassifier
from alfred.nn.focal_loss import BinaryFocalLoss
from alfred.utils import model_util


class Model(base.Model):
    def __init__(self, args, embs_ann, vocab_out, pad, seg):
        '''
        transformer agent
        '''
        super().__init__(args, embs_ann, vocab_out, pad, seg)

        # encoder and visual embeddings
        self.encoder_vl = EncoderVL(args)
        # pre-encoder for language tokens
        self.encoder_lang = EncoderLang(
            args.encoder_lang['layers'], args, embs_ann)
        # feature embeddings
        self.vis_feat = FeatureFlat(
            input_shape=self.visual_tensor_shape,
            output_size=args.demb)
        # dataset id learned encoding (applied after the encoder_lang)
        self.dataset_enc = None
        if args.enc['dataset']:
            self.dataset_enc = DatasetLearnedEncoding(args.demb, args.data['train'])
        # embeddings for actions
        self.emb_action = nn.Embedding(len(vocab_out), args.demb)
        # dropouts
        self.dropout_action = nn.Dropout2d(args.dropout['transformer']['action'])

        # decoder parts
        encoder_output_size = args.demb
        self.dec_action = nn.Linear(encoder_output_size*2, args.demb)
        self.dec_object = ObjectClassifier(encoder_output_size)

        proj_dim = 512
        self.img_proj = nn.Linear(encoder_output_size, proj_dim)
        self.txt_proj = nn.Linear(encoder_output_size, proj_dim)

        self.dec_boundary = nn.Linear(encoder_output_size, 1)

        self.feat_head = nn.Linear(encoder_output_size, encoder_output_size)

        self.log_scale = nn.Parameter(torch.Tensor([0.0]), requires_grad=True)
        # for meta action
        self.meta_action_vocab = torch.load('data/lmdb_human/data.vocab')['meta_action_vocab']
        self.emb_meta_action = nn.Embedding(len(self.meta_action_vocab), args.demb)
        self.dec_meta_action = nn.Linear(encoder_output_size, args.demb)

        # skip connection for object predictions
        self.object_feat = FeatureFlat(
            input_shape=self.visual_tensor_shape,
            output_size=args.demb)

        # progress monitoring heads
        if self.args.progress_aux_loss_wt > 0:
            self.dec_progress = nn.Linear(encoder_output_size, 1)
        if self.args.subgoal_aux_loss_wt > 0:
            self.dec_subgoal = nn.Linear(encoder_output_size, 1)

        # final touch
        self.register_buffer('count', torch.zeros(1), persistent=False)
        self.init_weights()
        self.reset()

    def forward(self, vocab, inference=False, **inputs):
        '''
        forward the model for multiple time-steps (used for training)
        '''
        # embed language
        output = {}
        emb_lang, lengths_lang = self.embed_lang(inputs['lang'], vocab)
        emb_lang = self.dataset_enc(emb_lang, vocab) if self.dataset_enc else emb_lang

        # embed frames and actions
        emb_frames, emb_object = self.embed_frames(inputs['frames'])
        lengths_frames = inputs['lengths_frames']
        emb_actions = self.embed_actions(inputs['action'])
        assert emb_frames.shape == emb_actions.shape, (emb_frames.shape, emb_actions.shape)
        lengths_actions = lengths_frames.clone()
        length_frames_max = inputs['length_frames_max']

        # concatenate language, frames and actions and add encodings
        encoder_out, _ = self.encoder_vl(
            emb_lang, emb_frames, emb_actions, lengths_lang,
            lengths_frames, lengths_actions, length_frames_max)
        # use outputs corresponding to visual frames for prediction only
        encoder_out_visual = encoder_out[
            :, lengths_lang.max().item():
            lengths_lang.max().item() + length_frames_max]

        # get the output actions
        decoder_input = encoder_out_visual.reshape(-1, self.args.demb)

        meta_action_emb_flat = self.dec_meta_action(decoder_input)
        meta_boundary = self.dec_boundary(decoder_input)
        meta_action_flat = meta_action_emb_flat.mm(self.emb_meta_action.weight.t())
        meta_action = meta_action_flat.view(
            *encoder_out_visual.shape[:2], *meta_action_flat.shape[1:])
        output.update({'meta_action': meta_action, 'meta_boundary': meta_boundary})

        meta_action_one_hot = F.gumbel_softmax(meta_action_flat, tau=1, hard=True)
        meta_action_feat = meta_action_one_hot.mm(self.emb_meta_action.weight)
        action_emb_flat = self.dec_action(torch.cat([decoder_input, meta_action_feat], dim=-1))
        action_flat = action_emb_flat.mm(self.emb_action.weight.t())
        action = action_flat.view(
            *encoder_out_visual.shape[:2], *action_flat.shape[1:])

        if not inference:
            instr_emb, contrastive_label = self.get_inst_emb(emb_lang, inputs['lang'], inputs['meta_sentnum'])
            img_feat = self.img_proj(decoder_input)
            txt_feat = self.txt_proj(instr_emb)
            proj_img = F.normalize(img_feat, p=2, dim=-1)
            proj_txt = F.normalize(txt_feat, p=2, dim=-1)
            contrastive_logits = torch.matmul(proj_img, proj_txt.transpose(0, 1))
            contrastive_logits = contrastive_logits / self.log_scale.exp()
            contrastive_label = contrastive_label.reshape(-1)
            if instr_emb.shape[0] <= contrastive_label.max():
                output.update({'contrastive_loss': torch.zeros(contrastive_logits.shape[0], device=contrastive_logits.device)})
                output.update({'correct_contrastive': torch.tensor(0.0 , device=contrastive_logits.device)})
            else:
                contrastive_loss = F.cross_entropy(contrastive_logits, contrastive_label, reduction='none')
                output.update({'contrastive_loss': contrastive_loss})

                pred_contrastive_label = torch.argmax(contrastive_logits, dim=-1)
                correct_contrastive = (pred_contrastive_label == contrastive_label).float().mean()
                output.update({'correct_contrastive': correct_contrastive}) 
            
        pred_feat = self.feat_head(decoder_input)
        pred_feat = pred_feat.view(
            *encoder_out_visual.shape[:2], *pred_feat.shape[1:])
        output.update({'pred_feat': pred_feat[:, :-1], 'emb_frames': emb_frames[:, 1:].detach()})

        # get the output objects
        emb_object_flat = emb_object.view(-1, self.args.demb)
        decoder_input = decoder_input + emb_object_flat
        object_flat = self.dec_object(decoder_input)
        objects = object_flat.view(
            *encoder_out_visual.shape[:2], *object_flat.shape[1:])
        output.update({'action': action, 'object': objects})

        # (optionally) get progress monitor predictions
        if self.args.progress_aux_loss_wt > 0:
            progress = torch.sigmoid(self.dec_progress(encoder_out_visual))
            output['progress'] = progress
        if self.args.subgoal_aux_loss_wt > 0:
            subgoal = torch.sigmoid(self.dec_subgoal(encoder_out_visual))
            output['subgoal'] = subgoal
        return output

    def get_inst_emb(self, emb_lang, input_lang, sent_num):
        # instr = 20
        instr_emb = emb_lang[torch.where(input_lang == 20)]
        contrastive_label = torch.zeros(sent_num.shape, device=sent_num.device, dtype=torch.long)
        for i in range(sent_num.shape[0]):
            if i > 0:
                contrastive_label[i] = sent_num[i] + contrastive_label[i-1].max() + 1
            else:
                contrastive_label[i] = sent_num[i]
        
        return instr_emb, contrastive_label

    def embed_lang(self, lang_pad, vocab):
        '''
        take a list of annotation tokens and extract embeddings with EncoderLang
        '''
        assert lang_pad.max().item() < len(vocab)
        embedder_lang = self.embs_ann[vocab.name]
        emb_lang, lengths_lang = self.encoder_lang(
            lang_pad, embedder_lang, vocab, self.pad)
        if self.args.detach_lang_emb:
            emb_lang = emb_lang.clone().detach()
        return emb_lang, lengths_lang

    def embed_frames(self, frames_pad):
        '''
        take a list of frames tensors, pad it, apply dropout and extract embeddings
        '''
        self.dropout_vis(frames_pad)
        frames_4d = frames_pad.view(-1, *frames_pad.shape[2:])
        frames_pad_emb = self.vis_feat(frames_4d).view(
            *frames_pad.shape[:2], -1)
        frames_pad_emb_skip = self.object_feat(
            frames_4d).view(*frames_pad.shape[:2], -1)
        return frames_pad_emb, frames_pad_emb_skip

    def embed_actions(self, actions):
        '''
        embed previous actions
        '''
        emb_actions = self.emb_action(actions)
        emb_actions = self.dropout_action(emb_actions)
        return emb_actions

    def reset(self):
        '''
        reset internal states (used for real-time execution during eval)
        '''
        self.frames_traj = torch.zeros(1, 0, *self.visual_tensor_shape)
        self.action_traj = torch.zeros(1, 0).long()

    def step(self, input_dict, vocab, prev_action=None, last_action_timestep=None):
        '''
        forward the model for a single time-step (used for real-time execution during eval)
        '''
        frames = input_dict['frames']
        device = frames.device
        if last_action_timestep is not None:
            if last_action_timestep == 0:
                self.reset()
                prev_action = None
            else:
                self.action_traj = self.action_traj[:, :last_action_timestep-1]
                self.frames_traj = self.frames_traj[:, :last_action_timestep]

        if prev_action is not None:
            prev_action_int = vocab['action_low'].word2index(prev_action)
            prev_action_tensor = torch.tensor(prev_action_int)[None, None].to(device)
            self.action_traj = torch.cat(
                (self.action_traj.to(device), prev_action_tensor), dim=1)

        self.frames_traj = torch.cat(
            (self.frames_traj.to(device), frames[None]), dim=1)
        # at timestep t we have t-1 prev actions so we should pad them
        action_traj_pad = torch.cat(
            (self.action_traj.to(device),
             torch.zeros((1, 1)).to(device).long()), dim=1)
        # print(self.frames_traj.shape, self.action_traj.shape, action_traj_pad.shape)
        model_out = self.forward(
            vocab=vocab['word'],
            lang=input_dict['lang'],
            lengths_lang=input_dict['lengths_lang'],
            length_lang_max=input_dict['length_lang_max'],
            frames=self.frames_traj.clone(),
            lengths_frames=torch.tensor([self.frames_traj.size(1)]),
            length_frames_max=self.frames_traj.size(1),
            action=action_traj_pad,
            inference=True)
        step_out = {}
        for key, value in model_out.items():
            # return only the last actions, ignore the rest
            step_out[key] = value[:, -1:]
        return step_out

    def compute_batch_loss(self, model_out, gt_dict):
        '''
        loss function for Seq2Seq agent
        '''
        losses = dict()

        meta_action_wt = 1.0
        meta_boundary_wt = 1.0
        contrastive_wt = 1.0
        feat_wt = 5.0

        # action loss
        action_pred = model_out['action'].view(-1, model_out['action'].shape[-1])
        action_gt = gt_dict['action'].view(-1)
        pad_mask = (action_gt != self.pad)
        action_loss = F.cross_entropy(action_pred, action_gt, reduction='none')
        action_loss *= pad_mask.float()
        action_loss = action_loss.mean()
        # losses['action'] = action_loss * self.args.action_loss_wt

        # meta action loss
        meta_action_pred = model_out['meta_action'].view(-1, model_out['meta_action'].shape[-1])
        meta_action_gt = gt_dict['meta_action'].view(-1)
        pad_mask = (meta_action_gt != self.pad)
        meta_action_loss = F.cross_entropy(meta_action_pred, meta_action_gt, reduction='none')
        meta_action_loss *= pad_mask.float()
        meta_action_loss = meta_action_loss.mean()
        losses['meta_action'] = meta_action_loss * meta_action_wt

        meta_boundary_pred = model_out['meta_boundary'].view(-1)
        meta_boundary_gt = gt_dict['meta_boundary'].view(-1)
        pad_mask = (action_gt != self.pad)
        bce_loss = nn.BCEWithLogitsLoss(reduction='none')
        meta_boundary_loss = bce_loss(meta_boundary_pred, meta_boundary_gt.float())
        meta_boundary_loss *= pad_mask.float()
        meta_boundary_loss = meta_boundary_loss.mean()
        losses['meta_boundary'] = meta_boundary_loss * meta_boundary_wt

        # contrastive loss
        contrastive_loss = model_out['contrastive_loss']
        contrastive_loss *= pad_mask.float()
        contrastive_loss = contrastive_loss.mean()
        losses['contrastive'] = contrastive_loss * contrastive_wt

        # feat loss
        batch_size, seq_len, feat_dim = model_out['pred_feat'].shape
        pred_feat = model_out['pred_feat'].reshape(-1, feat_dim)
        gt_feat = model_out['emb_frames'].reshape(-1, feat_dim)
        binary_logits = torch.matmul(pred_feat, gt_feat.t())
        triu_mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=0)
        triu_mask = triu_mask.expand(batch_size, seq_len, seq_len)
        binary_label = torch.block_diag(*triu_mask).to(pred_feat.device)
        criterion = BinaryFocalLoss(gamma=2, alpha=0.75)
        feat_loss = criterion(binary_logits, binary_label)
        feat_loss = feat_loss.mean(dim=1)
        feat_loss_mask = gt_dict['action'][:, :-1]
        feat_loss_mask = feat_loss_mask.reshape(-1)
        feat_loss_mask = (feat_loss_mask != self.pad).float()
        feat_loss *= feat_loss_mask
        feat_loss = feat_loss.mean()
        # losses['feat'] = feat_loss * feat_wt

        # object classes loss
        object_pred = model_out['object']
        object_gt = torch.cat(gt_dict['object'], dim=0)
        interact_idxs = gt_dict['action_valid_interact'].view(-1).nonzero(
            as_tuple=False).view(-1)
        if interact_idxs.nelement() > 0:
            object_pred = object_pred.view(
                object_pred.shape[0] * object_pred.shape[1],
                *object_pred.shape[2:])
            object_loss = model_util.obj_classes_loss(
                object_pred, object_gt, interact_idxs)
            losses['object'] = object_loss * self.args.object_loss_wt

        # subgoal completion loss
        if self.args.subgoal_aux_loss_wt > 0:
            subgoal_pred = model_out['subgoal'].squeeze(2)
            subgoal_gt = gt_dict['subgoals_completed']
            subgoal_loss = F.mse_loss(subgoal_pred, subgoal_gt, reduction='none')
            subgoal_loss = subgoal_loss.view(-1) * pad_mask.float()
            subgoal_loss = subgoal_loss.mean()
            losses['subgoal_aux'] = self.args.subgoal_aux_loss_wt * subgoal_loss

        # progress monitoring loss
        if self.args.progress_aux_loss_wt > 0:
            progress_pred = model_out['progress'].squeeze(2)
            progress_gt = gt_dict['goal_progress']
            progress_loss = F.mse_loss(progress_pred, progress_gt, reduction='none')
            progress_loss = progress_loss.view(-1) * pad_mask.float()
            progress_loss = progress_loss.mean()
            losses['progress_aux'] = self.args.progress_aux_loss_wt * progress_loss

        # maximize entropy of the policy if asked
        if self.args.entropy_wt > 0.0:
            policy_entropy = - F.softmax(
                action_pred, dim=1) * F.log_softmax(action_pred, dim=1)
            policy_entropy = policy_entropy.mean(dim=1)
            policy_entropy *= pad_mask.float()
            losses['entropy'] = - policy_entropy.mean() * self.args.entropy_wt
        
        if self.count % 100 == 0:
            print(losses)
            print(model_out['correct_contrastive'].mean().item())
        self.count += 1

        return losses

    def init_weights(self, init_range=0.1):
        '''
        init embeddings uniformly
        '''
        super().init_weights(init_range)
        self.dec_action.bias.data.zero_()
        self.dec_action.weight.data.uniform_(-init_range, init_range)
        self.emb_action.weight.data.uniform_(-init_range, init_range)

    def compute_metrics(self, model_out, gt_dict, metrics_dict, verbose=False):
        '''
        compute exact matching and f1 score for action predictions
        '''
        preds = model_util.extract_action_preds(
            model_out, self.pad, self.vocab_out, lang_only=True)
        stop_token = self.vocab_out.word2index('<<stop>>')
        gt_actions = model_util.tokens_to_lang(
            gt_dict['action'], self.vocab_out, {self.pad, stop_token})
        model_util.compute_f1_and_exact(
            metrics_dict, [p['action'] for p in preds], gt_actions, 'action')
        
        # for meta action
        meta_action_preds = self.process_meta_action_output(model_out)
        stop_token = self.meta_action_vocab.word2index('stop')
        gt_meta_actions = model_util.tokens_to_lang(
            gt_dict['meta_action'], self.meta_action_vocab, {self.pad, stop_token})
        model_util.compute_f1_and_exact(
            metrics_dict, meta_action_preds, gt_meta_actions, 'meta_action')

        # for meta boundary
        meta_boundary_preds = model_out['meta_boundary'].argmax(dim=1)
        meta_boundary_gts = gt_dict['meta_boundary'].view(-1)
        nonpad_mask = gt_dict['action'].view(-1) != self.pad
        meta_boundary_correct = meta_boundary_preds == meta_boundary_gts
        meta_boundary_correct = meta_boundary_correct * nonpad_mask
        meta_boundary_correct = meta_boundary_correct.float().sum()
        meta_boundary_correct /= torch.sum(nonpad_mask).float()
        metrics_dict['meta_boundary'].append(meta_boundary_correct.item())

        correct_contrastive = model_out['correct_contrastive'].mean()
        metrics_dict['contrastive'].append(correct_contrastive.item())

        # for feat contrast
        batch_size, seq_len, feat_dim = model_out['pred_feat'].shape
        pred_feat = model_out['pred_feat'].reshape(-1, feat_dim)
        gt_feat = model_out['emb_frames'].reshape(-1, feat_dim)
        binary_logits = torch.matmul(pred_feat, gt_feat.t())
        triu_mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=0)
        triu_mask = triu_mask.expand(batch_size, seq_len, seq_len)
        binary_label = torch.block_diag(*triu_mask).to(pred_feat.device)
        binary_pred = binary_logits > 0
        binary_correct = binary_pred == binary_label
        binary_correct = binary_correct.float() * binary_label.float()
        binary_correct = binary_correct.float().sum() / binary_label.float().sum()
        metrics_dict['feat_contrast'].append(binary_correct.item())


        # for object
        model_util.compute_obj_class_precision(
            metrics_dict, gt_dict, model_out['object'])

    def process_meta_action_output(self, model_out):
        '''
        process meta action output
        '''
        meta_action_preds = model_out['meta_action'].max(2)[1].tolist()
        return_list = []
        for i, action in enumerate(meta_action_preds):
            if 0 in action:
                pad_start_idx = action.index(0)
                action = action[:pad_start_idx]
                stop_token = self.meta_action_vocab.word2index('stop')
                if stop_token in action:
                    stop_start_idx = action.index(stop_token)
                    action = action[:stop_start_idx]
            # index to API actions
            words = self.meta_action_vocab.index2word(action)
            return_list.append(' '.join(words))
            

        return return_list

