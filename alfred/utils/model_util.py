...
class LinearWarmupScheduler(_LRScheduler):
    def __init__(self, optimizer, warmup_iterations, decay_iterations, base_lr):
        self.warmup_iterations = warmup_iterations
        self.decay_iterations = decay_iterations
        self.base_lr = base_lr
        print(warmup_iterations, decay_iterations, base_lr)
        super().__init__(optimizer)

    def get_lr(self):
        if self.last_epoch < self.warmup_iterations:
            warmup_factor = self.last_epoch / self.warmup_iterations
            return [base_lr * warmup_factor for base_lr in self.base_lrs]
        elif self.last_epoch > self.decay_iterations:
            return [self.base_lr * 0.1 for base_lr in self.base_lrs] 
        else:
...
def create_optimizer_and_schedulers(first_epoch, args, parameters, optimizer=None, epoch_length=1000):
    '''
    ...
    assert args.lr['profile'] in ('linear', 'cosine', 'triangular', 'triangular2', 'linearwarmup')
    ...
    elif args.lr['profile'] == 'linearwarmup':
        lr_scheduler = LinearWarmupScheduler(optimizer, epoch_length*2, epoch_length*args.lr['decay_epoch'], init_lr)
    else:
...
def generate_attention_mask(len_lang, len_frames, device, num_input_actions=0):
    ...
    # 2.3.1 attend to all prev actions
    # frames_to_actions = triangular_mask(len_frames, device=device, diagonal_shift=0)
...
def extract_meta_action_preds(model_out, pad, vocab):
    ...
    for i, action in enumerate(meta_action_preds):
        if pad in action:
            pad_start_idx = action.index(pad)
            action = action[:pad_start_idx]
            stop_token = vocab.word2index('stop')
            if stop_token in action:
                stop_start_idx = action.index(stop_token)
                action = action[:stop_start_idx]
        # index to API actions
        words = vocab.index2word(action)
...
def last_model_path(exp_name):
    ...
    '''
    model_path = os.path.join(constants.LACMA_LOGS, exp_name, 'latest.pth')
